...
class AuthorCreateTest(TestCase):
    ...
    def test_if_logged_in_with_permission(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_all_borrowed_book_list_on_success(self):
        ...
        author_date_of_birth = '02/12/1999'

        response = self.client.post(reverse('author_create'), {'first_name': author_first_name, 'last_name': author_last_name, 'date_of_birth': author_date_of_birth})
